...
def run_job_b():
    print("=== STARTING JOB B: CAST / CHARACTER REFERENCE (IP-ADAPTER) ===")
    ...
    print(f"Reference audit: {json.dumps(compiled.get('audit_trail', {}).get('reference'), indent=2)}")
    ...
    out_path = os.path.join("scratch", "job_b_reference_result.png")
    with open(out_path, "wb") as f:
        f.write(img_bytes)
    print(f"Result image saved to {out_path} ({len(img_bytes)} bytes)")
    
    # Save job record metadata
    meta_path = os.path.join("scratch", "job_b_reference_metadata.json")
